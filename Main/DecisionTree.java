//André João Pinto Couto. 202004160 FCUP 2022
import java.io.*;
import java.util.*;
...
//Pertinent info about Tree Structure
	 ...
	 *  nodes) or class number (for leaf nodes) in value, and an array of
	 *  tree nodes in children containing each of the children of the
	 ...
	 *  Strings in strings[][]. EX: if value == 3, then the array of
	...
	class TreeNode {
		...
		String toString(String indent) {
			if (children != null) {
				...
				return indent + "Class: " + strings[attributes-1][value] + " | Count: " + count + "\n";
		} // toString(String)
...
//Classify. Classifies the test data after training, via traverse method
	...
	public void classify(String[][] testData) {
		...
		for(int i = 1; i<testData.length; i++) {
			...
			System.out.println("Classification: "+ans); // Output classification to console which can be then be mapped to a file using "> xyz.file"
		}
...
//Tree Builder (Main bit of work)	
	...
	* Each call on buildTree will split the dataset on
	* the best attribute. The current node that is set to be split will have it's value
	* the same as the best attribute's value. It's children are then added with their respective indexes.
...
//Calculates dataset entropy, assuming last column represents class	
	/**
	* Pass the dataset we want to calculate the entropy from. Assumes last column is class
	* Uses stringCount and manipulates it in order to get the right number of
	* columns and rows
	**/
